...
Subsumption & For $i < j$, $\Mi$ simulated by $M_j$ with $\log$ overhead \\
...
For the machine hierarchy $\mathcal{M}$: (1) $\Mi$ can decide any language in $\DSPACE(2^i)$, (2) There exist languages decidable by $M_{i+1}$ but not by $\Mi$, (3) $\bigcup_{i=1}^{\infty} L(\Mi) = \textsf{RE}$ (recursively enumerable), (4) The hierarchy is strict: $L(\Mi) \subsetneq L(M_{i+1})$ for all $i$.

\begin{proof}[Proof sketch]
(1) By construction, $\Mi$ has $2^i$ bits. (2) By space hierarchy theorem. (3) Any TM computation can be simulated by sufficiently large $n$. (4) Follows from (2).
\end{proof}
\end{proposition}
