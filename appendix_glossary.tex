...
\midrule
Gap Function ($f(n)$) & Variable positive integer function determining resource jumps in hierarchy: $M_n \subseteq M_{n+f(n)}$. May be constant, logarithmic, polynomial, or exponential. Larger $f(n)$ represents qualitative shifts in processing level. Universality requires $\lim_{n \to \infty} f(n) = \infty$. \\
\midrule
...
\midrule
Hierarchy & In our framework, the sequence of finite machines $M_1, M_2, \ldots, M_n$ with exponentially growing resources ($2^n$ bits for $M_n$). Exhibits generalized non-uniform structure: $M_n \subseteq M_{n+f(n)}$ where $f(n) \geq 1$ represents variable resource gaps. Not present in single-level theories. \\
\midrule
...
\midrule
Machine Hierarchy & Sequence $\mathcal{M} = (M_i)_{i=1}^{n_{\max}}$ where $M_i$ is a finite state machine with $2^i$ bits of memory. Core computational architecture with non-uniform inclusion: $M_n \subseteq M_{n+f(n)}$ where gap function $f(n) \geq 1$ varies. \\
\midrule
...
$f(n)$ & Gap function: variable resource jump $M_n \subseteq M_{n+f(n)}$ \\
...
$M_n \subseteq M_{n+f(n)}, f(n) \geq 1$ & Non-uniform hierarchy inclusion \\
