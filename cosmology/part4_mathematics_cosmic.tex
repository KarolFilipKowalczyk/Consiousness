...
The selector function $S$ is the mechanism by which computational collapse occurs across all scales. This section provides a rigorous mathematical formalization, addressing the fundamental tension: $S$ must be powerful enough to select across the transfinite hierarchy, yet cannot be computable.

\subsection{Formal Definition of the Selector}

\begin{definition}[Selector Function]\label{def:selector}
The selector $S$ is a partial function:
\begin{equation}
S: \mathcal{P} \times \mathcal{H} \rightharpoonup \text{Ord} \times \mathcal{S}
\end{equation}
where:
\begin{itemize}
\item $\mathcal{P}$ = space of computational problems
\item $\mathcal{H}$ = history of prior collapses (context)
\item $\text{Ord}$ = class of ordinal numbers (specifying machine level)
\item $\mathcal{S}$ = space of solutions
\item $\rightharpoonup$ denotes partial function (not defined for all inputs)
\end{itemize}

For a given problem $p \in \mathcal{P}$ and history $h \in \mathcal{H}$, when defined, $S(p, h)$ returns:
\begin{enumerate}
\item An ordinal $\alpha \in \text{Ord}$ specifying which machine level $M_\alpha$ to deploy
\item A specific solution $s \in \mathcal{S}_\alpha$ from among the parallel explorations of $M_\alpha$
\end{enumerate}
\end{definition}

\begin{remark}[Proper Class]
The selector $S$ is not a set but a \emph{proper class}—it operates over the entire class of ordinals $\text{Ord}$. This is mathematically necessary: if $S$ were a set at some level $V_\kappa$ in the cumulative hierarchy, it could not select for machines at levels $\alpha \geq \kappa$.
\end{remark}

\subsection{Non-Computability of the Selector}

The selector must be non-computable at every level to avoid contradictions.

\begin{theorem}[Selector Transcendence]\label{thm:selector-noncomputable}
For any ordinal $\alpha$, there exists no machine $M_\beta$ (for any $\beta$) that computes $S$ restricted to problems at level $\alpha$.
\end{theorem}

\begin{proof}
We prove this in three cases covering all possibilities.

\textbf{Case 1: $\beta < \alpha$}

Suppose $M_\beta$ computes $S_\alpha$ (selector restricted to level $\alpha$ problems). Let $\mathcal{P}_\alpha$ be the possibility space at level $\alpha$, with $|\mathcal{P}_\alpha| = \aleph_\alpha$.

Machine $M_\beta$ has $|Q_\beta| = \aleph_\beta < \aleph_\alpha$ states. It cannot represent all possibilities in $\mathcal{P}_\alpha$, hence cannot compute a function $S_\alpha: \mathcal{P}_\alpha \times \mathcal{H} \to \mathcal{P}_\alpha$. 

By cardinality: $M_\beta$ can represent at most $\aleph_\beta$ distinct possibilities, but must select among $\aleph_\alpha > \aleph_\beta$ possibilities. Contradiction.

\textbf{Case 2: $\beta = \alpha$}

Suppose $M_\alpha$ computes its own selector $S_\alpha$. Consider the diagonal problem:

Define problem $P_{\text{diag}}$ as follows. Given input $\langle M, x \rangle$ where $M$ is a machine description at level $\alpha$:
\begin{itemize}
\item Compute what $S_\alpha$ predicts $M$ will output on $x$
\item If prediction is "accept", output "reject"
\item If prediction is "reject", output "accept"
\end{itemize}

Now ask: What does $M_\alpha$ output on input $\langle M_\alpha, P_{\text{diag}} \rangle$?

If $M_\alpha$ computes $S_\alpha$ correctly:
\begin{itemize}
\item $S_\alpha$ predicts what $M_\alpha$ will do on this input
\item But by construction, $P_{\text{diag}}$ does the opposite of the prediction
\item This is a logical contradiction
\end{itemize}

This is the classic diagonal argument (analogous to Turing's halting problem proof). Therefore, $M_\alpha$ cannot compute its own selector.

\textbf{Case 3: $\beta > \alpha$}

Suppose $M_\beta$ (with $\beta > \alpha$) computes $S_\alpha$. While $M_\beta$ has sufficient states ($\aleph_\beta > \aleph_\alpha$), the selector must operate on the \emph{entire} hierarchy.

Consider: If $S_\alpha$ is computable at level $\beta$, then by the same argument, $S_\beta$ should be computable at some level $\gamma > \beta$. By induction, every $S_\kappa$ is computable at some higher level.

But this leads to infinite regress: to compute any $S_\alpha$, we need the entire infinite tower $M_{\alpha+1}, M_{\alpha+2}, \ldots$. There is no "top" of this tower (ordinals are unbounded), so no single machine can compute $S$.

More precisely: The computational power needed to compute $S$ exceeds any ordinal level, placing $S$ outside the hierarchy entirely.

Therefore, $S$ is non-computable at every level $\alpha$.
\end{proof}

\begin{corollary}[Selector Beyond Computation]
The selector $S$ performs \emph{hypercomputation}—it solves problems that no machine in the transfinite hierarchy can solve, including halting problems at every level.
\end{corollary}

\subsection{Computable Approximation of the Selector}

Although $S$ itself is non-computable, we can define computable approximations that converge to its behavior.

\begin{definition}[Bounded Selector Approximation]\label{def:selector-approx}
For each $k \in \mathbb{N}$, define the $k$-bounded approximation:
\begin{equation}
S_k: \mathcal{P} \times \mathcal{H} \to \mathbb{N} \times \mathcal{S}
\end{equation}
as:
\begin{equation}
S_k(p, h) = \argmin_{n \leq k} \left\{ K_k(M_n(p)|h) + \lambda \log n \right\}
\end{equation}
where:
\begin{itemize}
\item $K_k(x|y)$ = $k$-bounded Kolmogorov complexity (computable)
\item $M_n(p)$ = solution produced by machine $M_n$ on problem $p$
\item $\lambda > 0$ = regularization parameter penalizing higher machine levels
\item $\argmin$ selects the machine level minimizing the objective
\end{itemize}
\end{definition}

\begin{remark}[Why This is Computable]
The approximation $S_k$ is computable because:
\begin{enumerate}
\item It only searches over finitely many machine levels ($n \leq k$)
\item Bounded Kolmogorov complexity $K_k$ is computable (unlike unbounded $K$)
\item The optimization is over a finite domain
\item Each $M_n$ simulation runs for at most $k$ steps
\end{enumerate}
However, $S_k$ gives incorrect answers for problems requiring machines $M_n$ with $n > k$.
\end{remark}

\begin{theorem}[Approximation Convergence]\label{thm:selector-convergence}
For any fixed problem $p$ and history $h$ where $S(p,h)$ is defined with $S(p,h) = (\alpha, s)$:
\begin{equation}
\lim_{k \to \infty} S_k(p, h) = (\alpha, s)
\end{equation}
with convergence rate:
\begin{equation}
\| S_k(p,h) - S(p,h) \| = O\left(\frac{1}{\log k}\right)
\end{equation}
for typical problems.
\end{theorem}

\begin{proof}[Proof sketch]
For sufficiently large $k > \alpha$:
\begin{enumerate}
\item $S_k$ can access machine $M_\alpha$ (since $\alpha < k$)
\item The $k$-bounded complexity $K_k$ approximates true complexity $K$ with error $O(1/k)$
\item The minimization finds the correct level $\alpha$ once $k$ is large enough
\item The error decreases as $k$ increases, with rate determined by complexity approximation error
\end{enumerate}

Detailed analysis (omitted) shows convergence rate $O(1/\log k)$ for problems with polynomial-time solutions at level $\alpha$.
\end{proof}

\begin{corollary}[Practical Computability]
For finite problems requiring machine levels $M_n$ with $n < 100$, the approximation $S_{1000}$ gives correct answers with high probability.
\end{corollary}

\subsection{Selector Weighting Function}

The selector does not choose uniformly among solutions—it has preferences.

\begin{definition}[Selector Weighting Function]
Given a set of candidate solutions $\{s_i\}$ at level $\alpha$, the selector chooses according to weights:
\begin{equation}
w_S(s_i) = \exp\left(\beta_1 \Phi(s_i) - \beta_2 K(s_i) + \beta_3 I(s_i|h)\right)
\end{equation}
where:
\begin{itemize}
\item $\Phi(s_i)$ = integrated information of solution $s_i$
\item $K(s_i)$ = Kolmogorov complexity of solution
\item $I(s_i|h)$ = mutual information with history $h$
\item $\beta_1, \beta_2, \beta_3 > 0$ = coupling constants
\end{itemize}

The probability of selecting solution $s_i$ is:
\begin{equation}
P(s_i) = \frac{w_S(s_i)}{\sum_j w_S(s_j)}
\end{equation}
\end{definition}

\begin{remark}[Interpretation]
This weighting function encodes the selector's "preferences":
\begin{itemize}
\item $\beta_1 > 0$: Prefer high integrated information (consciousness-supporting)
\item $\beta_2 > 0$: Prefer low Kolmogorov complexity (simplicity bias)
\item $\beta_3 > 0$: Prefer coherence with history (temporal consistency)
\end{itemize}
These are the fundamental "laws" governing which possibilities actualize.
\end{remark}

\begin{lemma}[Approximation Error Bound]\label{lem:approx-error}
The error in approximating $S$ by $S_k$ satisfies:
\begin{equation}
\mathbb{E}\left[\left| S(p,h) - S_k(p,h) \right|\right] \leq \frac{C \log(|p|)}{\log k}
\end{equation}
for problems of size $|p|$ and constant $C$ depending on the problem class.
\end{lemma}

\begin{proof}
The error comes from two sources:
\begin{enumerate}
\item \textbf{Complexity approximation error:} $K_k(x) - K(x) = O(\log k)$ for most strings
\item \textbf{Level truncation error:} If true answer requires $n > k$, error is large; but this probability decreases as $O(1/k)$ for typical problems
\end{enumerate}

Combining these and taking expectation over problem distribution yields the stated bound.
\end{proof}
